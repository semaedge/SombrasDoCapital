 ...
 * Componente: 19_ConsequenceService.gs
 ...
 *
 * INTEGRAÇÕES
 * Calcula efeitos éticos, jornalísticos, ambientais e narrativos de cada escolha.
 *
 * CONTRATO DE DADOS
...
function ConsequenceService_healthcheck() {
  return { component: '19_ConsequenceService.gs', status: 'implemented' };
}

function evaluateChoice(choice) {
  var action = normalizeIdentifier(choice);
  if (action === 'inaction') {
    return {
      choice: action,
      text: 'O prazo terminou sem decisão. A inação preservou o rigor, mas atrasou a investigação.',
      impact: { speed: -2, documentaryRigor: 0 }
    };
  }
  if (action === 'verify') {
    return {
      choice: action,
      text: 'Boa decisão: a investigação avançou com uma segunda checagem de fonte.',
      impact: { speed: -1, documentaryRigor: 2 }
    };
  }
  return {
    choice: action,
    text: 'A publicação ganhou velocidade, mas a falta de confirmação deixou uma dúvida registrada.',
    impact: { speed: 2, documentaryRigor: -2 }
  };
}
